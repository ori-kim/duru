...
export { createMemoryStore, createMemoryPlugin, installMemoryCommands, memory, memoryCommandsPlugin };
...
function createMemoryPlugin(options: MemoryPluginOptions = {}) {
  return virtualPlugin((cli) => installMemoryCommands(cli, options));
}

function memory(options: MemoryPluginOptions = {}) {
  return createPlugin((api) => installMemoryCommands(api.cli, options));
}

const memoryCommandsPlugin = memory;

async function installMemoryCommands(cli: Cli, options: MemoryPluginOptions = {}): Promise<void> {
  ...
  memory
    ...
    .action(async (ctx) => {
      ...
      const indexing = result.removed.length > 0 ? await maybeScheduleReindex(qmd, store, options) : skippedIndexing();
