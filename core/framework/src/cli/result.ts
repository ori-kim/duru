import { isExitResult } from "../result/index.ts";
import type { CliEventRecord, Context, HelpDocument, RenderInput, RoutePresenter } from "../types/index.ts";
...
export function helpPresenters(document: HelpDocument, text: string): ReadonlyMap<string, RoutePresenter<unknown>> {
  const presenters = new Map<string, RoutePresenter<unknown>>();
  presenters.set("text", () => text);
  presenters.set("json", () => document);
  return presenters;
}
