...
describe("history extension CLI flow", () => {
  test("records a clip run through command-end and exposes it through clip history", async () => {
    ...
    const list = await runClip({ home, manifest, args: ["list"] });
    ...
    expect(parsed.records[0]?.command.kind).toBe("command");
    ...
    expect(table.stdout).toContain("clip list");
    ...
    expect(show.stdout).toContain("command: clip list");
  });
