...
describe("extension-loader / Phase 2 lazy init", () => {
  test("argv='list' — extension metadata 조회를 위해 skills extension도 import됨", async () => {
    ...
    process.env.CLIP_EXT_MANIFEST = undefined;
  });
  ...
  test("hooks 선언 extension은 argv 무관하게 항상 import 됨 (eager)", async () => {
    ...
    const manifestPath = createTmpManifest(dir, [{ name: "hooks-ext", path: extDir, hooks: true }]);
