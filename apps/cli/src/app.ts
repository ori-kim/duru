...
import { installVirtualPlugins } from "@duru/virtual-plugins";
...
async function createAppCliRuntime(argv?: string[]) {
  ...
  // DURU_HOME/plugins/plugins.yml 기반으로 플러그인을 동적 로드한다.
  // argv를 넘겨 contributes.commands 매칭으로 불필요한 플러그인 import를 건너뜀.
  // eager: true 플러그인(context-mode 등)은 항상 로드됨.
  await installVirtualPlugins(cli, { home: fileHome.root }, argv);
